...
def launch_setup(context, *args, **kwargs):
    ...
    mod = load_python_launch_file_as_module(os.path.join(get_package_share_directory('xarm_controller'), 'launch', 'lib', 'stevensIDM_controller_lib.py'))
    generate_ros2_control_params_temp_file = getattr(mod, 'generate_ros2_control_params_temp_file')
    ros2_control_params = generate_ros2_control_params_temp_file(
        os.path.join(get_package_share_directory('xarm_controller'), 'config', 'stevensIDM_controllers.yaml'.format(dof.perform(context))),
    ...
    robot_description = {
        'robot_description': get_xacro_file_content(
            xacro_file=PathJoinSubstitution([FindPackageShare('xarm_description'), 'urdf', 'stevensIDM_device.urdf.xacro']), 
            arguments={
                'prefix': prefix,
                'dof': dof,
