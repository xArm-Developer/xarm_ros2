...
def launch_setup(context, *args, **kwargs):
    ...
    mod = load_python_launch_file_as_module(os.path.join(get_package_share_directory(moveit_config_package_name), 'launch', 'lib', 'stevensARM_moveit_config_lib.py'))
    get_xarm_robot_description_parameters = getattr(mod, 'get_xarm_robot_description_parameters')
    robot_description_parameters = get_xarm_robot_description_parameters(
        xacro_urdf_file=PathJoinSubstitution([FindPackageShare('xarm_description'), 'urdf', 'stevensARM_device.urdf.xacro']),
        xacro_srdf_file=PathJoinSubstitution([FindPackageShare('xarm_moveit_config'), 'srdf', 'stevensARM.srdf.xacro']),
        urdf_arguments={
            'prefix': prefix,
    ...
    controllers_yaml = load_yaml(moveit_config_package_name, 'config', 'stevensARM', '{}.yaml'.format(controllers_name.perform(context)))
    ompl_planning_yaml = load_yaml(moveit_config_package_name, 'config', 'stevensARM', 'ompl_planning.yaml')
    kinematics_yaml = robot_description_parameters['robot_description_kinematics']
    joint_limits_yaml = robot_description_parameters.get('robot_description_planning', None)

    if add_gripper.perform(context) in ('True', 'true'):
        gripper_controllers_yaml = load_yaml(moveit_config_package_name, 'config', 'stevensIDM', '{}.yaml'.format(controllers_name.perform(context)))
        gripper_ompl_planning_yaml = load_yaml(moveit_config_package_name, 'config', 'stevensIDM', 'ompl_planning.yaml')
        gripper_joint_limits_yaml = load_yaml(moveit_config_package_name, 'config', 'stevensIDM', 'joint_limits.yaml')
